...
class Trainer:
    def __init__(self, data_loaders, dataset, parameter):
        ...
        self.eta=int(parameter['eta']*100)
        ...
        self.care = CARE(dataset, parameter, self.num_symbols, embed=self.symbol2emb, update_embeddings=self.update_embeddings)
        self.care.to(self.device)
        self.optimizer = torch.optim.Adam(self.care.parameters(), self.learning_rate)
    ...
    def load_embed(self):
        ...
        if self.embed_model in ['DistMult', 'TransE', 'ComplEx', 'RESCAL']:
            ...
            ent_update_embed = np.loadtxt(self.data_path + '/emb' + f'/entity2vec_degree{self.eta}.' + self.embed_model)
    ...
    def reload(self):
        ...
        if os.path.isfile(state_dict_file):
            self.care.load_state_dict(state)
        else:
            raise RuntimeError('No state dict in {}!'.format(state_dict_file))

    def save_checkpoint(self, epoch):
        torch.save(self.care.state_dict(), os.path.join(self.ckpt_dir, 'state_dict_' + str(epoch) + '.ckpt'))
    ...
    def write_training_log(self, data, epoch):
        self.loss_logger.info("Epoch: {}\tLoss_margin: {:.4f}\tLoss_ad: {:.4f}\r".format(epoch, data['Loss_margin'], data['Loss_ad']))
    ...
    def do_one_step(self, task, iseval=False, curr_rel='', istest=False):
        ...
        if not iseval:
            ...
            p_score, n_score, positive_context, negative_context, support_emb = self.care(task, iseval, curr_rel, support_meta, support_negative_meta)
            negative_context = torch.stack(negative_context, dim=1)
            y = torch.ones_like(p_score).to(self.device)
            loss_margin = self.care.loss_func(p_score, n_score, y)
            loss_adnce = ADNCE(negative_mode='paired')
            # support_emb:[bs*few,emd_dim]  [bs*few,emd_dim]    [bs*few,num_neg,emd_dim]
            loss_ad= loss_adnce(support_emb, positive_context, negative_context)
            loss_ad = 0.05 * loss_ad
            loss = loss_margin + loss_ad
            loss.backward()
            self.optimizer.step()
        elif curr_rel != '':
            p_score, n_score, _, _, _ = self.care(task, iseval, curr_rel, support_meta)
            y = torch.ones_like(p_score).to(self.device)
            loss_margin = self.care.loss_func(p_score, n_score, y)
            loss_ad = 0
        return loss_margin, loss_ad, p_score, n_score
    ...
    def train(self):
        ...
        for e in range(self.epoch):
            ...
            loss_margin, loss_ad, _, _ = self.do_one_step(train_task, iseval=False, curr_rel=curr_rel, istest=False)
            if e % self.print_epoch == 0:
                loss_margin = loss_margin.item()
                loss_ad = loss_ad.item()
                self.write_training_log({'Loss_margin': loss_margin, 'Loss_ad': loss_ad}, e)
